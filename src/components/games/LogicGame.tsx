  ...
  const handleCheck = useCallback(() => {
    ...
    if (isCorrect) {
      ...
      letterIndices.forEach(i => {
        ...
        if (typed[i] === answerChars[i]) { newLocked.add(i); }
        else { newTyped[i] = ""; }
      });
      setLocked(newLocked);
      setTyped(newTyped);
    }
  }, [typed, locked, revealed, answerChars, letterIndices, hintsUsed, data.answers, gameId]); // eslint-disable-line
  ...
  const handleKey = useCallback((e: KeyboardEvent) => {
    ...
    if (key === "Backspace") {
      ...
      if (typed[activeIdx] && !locked.has(activeIdx) && !revealed.has(activeIdx)) {
        ...
        if (prev !== null) { const next = [...typed]; next[prev] = ""; setTyped(next); setActiveIdx(prev); }
      }
      return;
    }
    if (key.length === 1) {
          ...
          <div key={wi} className="flex justify-center gap-2" dir="rtl">
            {word.map(({ char, globalIdx }) => {
              ...
              const displayLetter =
                ...
                typed[globalIdx] || "";
              return (
                <button key={globalIdx} onClick={() => handleCellClick(globalIdx)} disabled={finished}
                  className={`w-12 h-14 rounded-xl border-2 text-xl font-bold transition-all flex items-center justify-center
                    ${state === "locked" ? "bg-green-500 border-green-500 text-white"
                    : state === "revealed" ? "bg-green-200 border-green-400 text-green-800"
                    : state === "active" ? "bg-brand-surface border-brand-accent text-brand-text shadow-sm shadow-brand-accent/30"
